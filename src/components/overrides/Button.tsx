...
import {
  ButtonMode,
  getButtonColors,
} from "react-native-paper/src/components/Button/utils";
...
import type { $Omit, ThemeProp } from "react-native-paper/src/types";
...
const Button = (
  ...
  const rippleColor =
    customRippleColor || color(textColor).alpha(0.25).rgb().string();
  ...
  const font = isV3 ? theme.fonts.labelLarge : theme.fonts.bodyMedium;
    ...
    >
      <TouchableRipple
        ...
        style={[
          ...
          {
            //@ts-expect-error
            paddingLeft: rawStyle?.paddingLeft || 0,
            //@ts-expect-error
            paddingRight: rawStyle?.paddingRight || 0,
          },
        ]}
        testID={testID}
        theme={theme}
